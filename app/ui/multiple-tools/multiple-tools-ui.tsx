...
const SUGGESTIONS = [
  "I'm in Mumbai. What's the weather like here?",
  "What's the weather in Tokyo? What's 42 × 17? And tell me a joke!",
  "Get the current time in NYC, roll a dice (1-6), and calculate 100 ÷ 7",
];

export default function MultipleToolsUI() {
  const [input, setInput] = useState("");
  const { messages, sendMessage, status, error } = useChat({
    transport: new DefaultChatTransport({ api: "/api/multiple-tools" }),
  });
  ...
  return (
    <div className="flex flex-col h-screen bg-[#050505] text-white font-sans selection:bg-indigo-500/30 overflow-hidden">
      ...
      >
        {(messages || []).length === 0 && !error && (
          <div className="h-full flex flex-col items-center justify-center text-center space-y-6 max-w-lg mx-auto opacity-80">
            <div className="w-20 h-20 rounded-3xl bg-indigo-500/5 border border-indigo-500/20 flex items-center justify-center">
              <Wrench className="w-10 h-10 text-indigo-500/50" />
            </div>
            <div>
              ...
              <p className="text-sm font-light mt-2 leading-relaxed text-white/60">
                Say your city in chat (e.g. &quot;I&apos;m in Mumbai&quot;), then
                ask &quot;What&apos;s the weather here?&quot; — the AI uses your
                chat context.
              </p>
            </div>
            <div className="flex flex-col gap-2 w-full max-w-md">
              ...
              {SUGGESTIONS.map((s, i) => (
                <button
                  ...
                  className="px-4 py-3 rounded-xl border border-white/10 bg-white/5 text-left text-sm text-white/80 hover:bg-white/10 hover:border-indigo-500/30 transition-colors"
                >
                  {s}
                </button>
              ))}
            ...
            >
              ...
              {m.role === "assistant" &&
                m.parts?.some(
                  ...
                  <div className="flex items-center gap-2 text-[10px] font-semibold uppercase tracking-widest text-indigo-400/70">
              ...
              <div className="text-[10px] text-white/20 font-mono tracking-tighter px-2 mt-1">
                {m.role === "assistant" ? "Multiple Tools" : "You"}
              </div>
            </div>
          </div>
        ))}
        ...
        {isLoading &&
          messages?.[messages.length - 1]?.role !== "assistant" && (
            <div className="flex items-start gap-4 animate-in fade-in duration-300">
              <div className="shrink-0 w-8 h-8 md:w-10 md:h-10 rounded-full bg-gradient-to-br from-indigo-500 to-violet-600 flex items-center justify-center text-xs font-bold shadow-lg shadow-black/20">
                AI
              </div>
      ...
      <footer className="relative z-20 p-4 md:p-6 bg-[#050505] border-t border-white/5 mt-auto">
        ...
        >
          <div className="absolute -inset-1 bg-gradient-to-r from-indigo-500 to-violet-600 rounded-2xl blur opacity-20 group-focus-within:opacity-40 transition duration-500" />
          <div className="relative flex items-center bg-zinc-900/90 border border-white/10 rounded-2xl backdrop-blur-xl overflow-hidden p-1.5">
            <input
              ...
              placeholder="Ask for weather, math, time, random number, joke..."
        ...
        <p className="text-[10px] text-center mt-3 text-white/20 font-light tracking-wide italic">
          State your city in chat, then ask &quot;weather here&quot; — all in the
          same conversation.
        </p>
      </footer>
